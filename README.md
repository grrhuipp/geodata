...
2. 产品、网站、产品内部子集和通用功能 category 进入数据集；以国家或地区为边界的 category、geolocation、国家入口和独立地区 TLD 汇总在同步阶段剔除。
3. 删除任何集合前，扫描整个 `data/` 中指向它的 `include:`，建立入站引用清单。
4. 若通用父 category 依赖待删除的地区子类，将子类中的有效产品引用和直接规则并入通用父类，再删除地区子类；带属性的 `include:` 应保持原有属性筛选语义。
5. 合并后重新扫描 `include:`，验证不存在缺失引用、循环引用或只有已删除子类引用的空 category。
6. 以处理前的已发布提交为基线比较 category 文件集合；逐项确认所有缺失项都符合地区汇总判定，所有通用功能 category 都仍存在。
7. 不把地区子类全文无条件复制到多个父类；只向实际依赖它的通用父类合并一次，后续由 `include:` 展开和构建器去重。
...
5. 保留产品、网站、产品内部子集和通用 `category-*`；按“数据范围判定”移除地区 category、`geolocation-*` 和独立国家/TLD 汇总。
6. 对删除文件先检查是否仍被本地 `include:` 引用。通用父 category 引用了地区子类时，先将有效内容并回父类。
7. 对照同步前已发布提交，确认缺少的 category 都符合地区汇总判定，通用功能 category 没有遗漏。
...
5. 保留通用 `category-*` 和产品内部地区子集；移除地区 category、`geolocation-*`、国家入口和独立国家/TLD 汇总；
6. 清理 category 前检查入站 `include:`，将仍有价值的内容并回通用父类，并以已发布提交为基线确认通用 category 没有遗漏；
