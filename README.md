...
- IP 规则只收录纯业务地址：必须能明确对应具体产品、网站或业务服务。ASN 前缀只在 ASN 由该业务产品直接持有且用于该业务时收录。
...
6. 官方没有完整网段、日志只有 ASN 线索时，只有在 ASN 由具体业务产品直接持有且用于该业务时，才从 RIPEstat `ris-prefixes` 获取当前起源前缀。
...
5. 对只有裸 IP 的记录，先检查该具体业务的官方地址清单；官方没有完整清单时，再确认是否属于该业务产品直接持有且用于该业务的 ASN。
6. 租户域名即使解析到 CDN 也只收录域名，不收录解析 IP。CDN、云平台、边缘网络和托管商地址不能作为兜底规则。
7. 运营商、住宅网、传输网络、共享托管和任何基础设施 ASN 全部跳过；无法证明为具体业务服务的裸 IP 不收录。
...
并非所有官方地址都应导入。只有能够明确对应具体业务产品或服务的 CIDR 才能加入；官方发布的单地址 CIDR 同样属于有效数据。共享云、CDN、边缘网络和托管基础设施地址即使有官方清单也不导入。
...
```text
V2Fly products plus local groups: 1511
Category groups: 44
Expanded rules: 68621
Official IP/CIDR source rules: 7663
RIPEstat ASN origin prefix source rules: 601
geodata.dat bytes: 1380380
geodata.dat SHA-256: 2605EBE789DDC8EB9B2A0571DEF1B479AD7F7136002CD6070225BE452453DED4
```
...
不作为基础设施规则收录。只有能够明确对应具体产品或业务服务的官方地址及产品自有
网络可以进入 IP 规则。无法确认纯业务归属的裸 IP、住宅网络、传输网络和临时解析
地址不收录。
...
3. 只选择明确的业务产品范围；不导入 CDN、云平台、边缘网络、托管商或其他共享基础设施地址，也不根据基础设施推测租户。
4. 将网络地址规范化；官方源中的 `/32`、`/128` 按原样保留。
5. 同一产品的重叠网段保留覆盖范围合理的最小集合。
6. 域名和 CIDR 放在同一个产品文件中，例如 `data/telegram`。
7. 仅对没有可用域名的裸 IP 目标读取 `target_asn`，并确认 ASN 由具体业务产品直接持有且用于该业务。
...
9. IP 只收录纯业务地址，优先采用产品官方 CIDR，并仅对具体业务产品直接持有且用于该业务的 ASN 使用 RIPEstat 起源前缀补全；CDN、云平台、边缘网络、托管商和运营商地址一律不收录，临时解析 IP 和来源不明单 IP 不使用，可信业务来源的 `/32`、`/128` 完整保留；
