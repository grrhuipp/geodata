...
- 发布数据保留产品、网站、产品子集和 `category-*` 集合；`geolocation-*`、国家入口及独立国家/TLD 地区汇总不进入成品。
...
[V2Fly/domain-list-community](https://github.com/v2fly/domain-list-community) 是域名规则的主要上游。同步时选取其产品文件、网站文件、产品子集和 `category-*` 集合，保留这些文件之间的 `include` 与属性语义，不把所有引用提前复制成文本；展开由编译器完成。`geolocation-*`、独立国家/TLD 和路由策略汇总在同步时剔除。
...
```text
V2Fly products plus local groups: 1544
Category groups: 115
Expanded rules: 172880
Official IP/CIDR source rules: 28014
RIPEstat ASN origin prefix source rules: 25015
geodata.dat bytes: 2877556
geodata.dat SHA-256: 094AA34DD4DDF1AF0FECD03BBF5548D11677AF8BC914CA4CBF0C81E242DEB55E
```
...
3. 提取产品、网站、能够用 `include:` 表达的产品子集和 `category-*`；剔除 `geolocation-*`、独立国家/TLD 和路由策略汇总。
